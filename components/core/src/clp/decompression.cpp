...
namespace clp {
    bool decompress_parquet_file (CommandLineArguments& command_line_args, const unordered_set<string>& files_to_decompress) {
        ...
        try {
            ...
            if (files_to_decompress.empty()) {
                for (boost::filesystem::directory_iterator iter(archives_dir); iter != boost::filesystem::directory_iterator(); ++iter) {
                    if (boost::filesystem::extension(iter->path()) == ".parquet") {
                        parquet_reader.open(iter->path());
                    } else {
                        SPDLOG_WARN("{} is not an archive", iter->path().filename().string());
                    }
                }
