...
ErrorCode Cursor::load_archive() {
    ...
    try {
        for (auto const& column : m_output_columns) {
            ...
            m_projection->add_column(ColumnDescriptor::create(descriptor_tokens));
        }
...
ErrorCode Cursor::execute_query(std::string& query, std::vector<std::string> output_columns) {
...
// size_t Cursor::fetch_next(size_t num_rows, std::vector<VectorPtr>& column_vectors) {
//     if (m_error_code != ErrorCode::Success) {
//         return 0;
//     }
//
//     while (m_current_archive_index != m_end_archive_index) {
//         while (m_current_schema_index != m_end_schema_index) {
...
//                     m_query_runner->add_wildcard_columns_to_searched_columns();
//
//                     auto& reader = m_archive_reader.read_table(m_current_schema_id, false,
//                     false); reader.initialize_query_runner(m_query_runner); m_error_code =
//                     ErrorCode::Success; m_current_schema_table_loaded = true;
//                 } else {
