...
void DictionaryReader<DictionaryIdType, EntryType>::read_entries(bool lazy) {
    ...
    auto& dictionary_file_reader = m_adaptor.checkout_reader_for_section(m_dictionary_path);

    uint64_t num_dictionary_entries;
    dictionary_file_reader.read_numeric_value(num_dictionary_entries, false);
    m_dictionary_decompressor.open(dictionary_file_reader, cDecompressorFileReadBufferCapacity);
